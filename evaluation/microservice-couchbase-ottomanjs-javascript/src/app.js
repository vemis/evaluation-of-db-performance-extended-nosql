...
async function startService() {
    ...
    app.listen(PORT, () => {
        console.log(`microservice-couchbase-ottomanjs-javascript running on port ${PORT}`);
        registerWithEureka();
    });
}
...
function registerWithEureka() {
    ...
    const hostname   = process.env.HOSTNAME || 'microservice-couchbase-ottomanjs-javascript';

    const client = new Eureka({
        instance: {
            app: 'microservice-couchbase-ottomanjs-javascript',
            hostName: hostname,
            ipAddr: hostname,
            port: { '$': PORT, '@enabled': 'true' },
            vipAddress: 'microservice-couchbase-ottomanjs-javascript',
            dataCenterInfo: {
